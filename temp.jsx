...
const SafeRoute = () => {
  ...
  useEffect(() => {
    ...
    const fetchCurrentLocation = async () => {
      try {
        ...
        setOrigin(`${newLocation.lat},${newLocation.lng}`);
        setError(null);
      } catch (err) {
  ...
  const handleMapLoad = (mapInstance) => {
    setMap(mapInstance);
    // Zoom to Shornur immediately when map loads
    mapInstance.setZoom(15); // Adjust zoom level as needed (15 shows city-level detail)
              ...
              >
                {!isMapLoading && (
                  <GoogleMap
                    ...
                    center={center}
                    zoom={15} // Initial zoom level to show Shornur in detail
